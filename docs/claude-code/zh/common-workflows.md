...
本页收集了日常开发的简明指南。关于提示词和上下文管理的更高层次指导，请参阅 [最佳实践](/zh/best-practices)。

本页涵盖：

* [提示词配方](#提示词配方)，用于探索代码、修复错误、重构、测试、PR 和文档编写
* [恢复之前的会话](#恢复之前的对话)，以便任务可以跨越多个时间段
* [使用工作树运行并行会话](#使用工作树运行并行会话)，避免并发编辑冲突
* [编辑前先规划](#编辑前先规划)，在更改写入磁盘前进行审阅
* [将研究委托给子代理](#将研究任务委托给子代理)，保持主上下文清晰
* [将 Claude 管道输入脚本](#定时运行-claude)，用于 CI 和批处理
...
关于在 monorepo 或大型代码库中配置 Claude Code，请参阅 [Monorepo 与大型仓库](/zh/large-codebases)。
...
假设你需要定位与特定功能或特性相关的代码。
  ...
  * 具体说明您要查找的内容
  * 使用项目中的领域语言
  * 为您的编程语言安装[代码智能插件](/zh/discover-plugins#code-intelligence)，让Claude能够精确进行“转到定义”和“查找引用”导航
...
当你使用 `gh pr create` 创建 PR 时，该会话会自动关联到该 PR。若需稍后返回此会话，可运行 `claude --from-pr <number>` 或将 PR URL 粘贴到 [`/resume` 选择器](/zh/sessions#use-the-session-picker) 的搜索栏中。
...
使用 @ 符号可以快速包含文件或目录，无需等待 Claude 读取它们。
    ...
    ```
    这通过格式 @server:resource 从已连接的 MCP 服务器获取数据。详见 [MCP 资源](/zh/mcp#use-mcp-resources)。
...
| 选项                                                 | 运行位置                     | 最适用于                                                                                                                                                                                                 |
| :----------------------------------------------------- | :-------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [例程](/zh/routines)                               | Anthropic 管理的基础设施  | 即使在电脑关闭时也应运行的任务。除了定时计划外，还可以在 API 调用或 GitHub 事件时触发。配置网址：[claude.ai/code/routines](https://claude.ai/code/routines)。 |
| [桌面定时任务](/zh/desktop-scheduled-tasks) | 您的机器，通过桌面应用 | 需要直接访问本地文件、工具或未提交更改的任务。                                                                                                                             |
| [GitHub Actions](/zh/github-actions)                   | 您的 CI 管道                  | 与仓库事件（如已打开的 PR）绑定的任务，或应与您的工作流配置一起存放的定时任务。                                                                                            |
| [`/loop`](/zh/scheduled-tasks)                         | 当前的 CLI 会话           | 会话保持打开状态时的快速轮询。当您开始新对话时任务会停止；`--resume` 和 `--continue` 可恢复未过期的任务。                                                                 |
...
claude --continue
```
这将恢复当前目录中最近的会话；如果尚未存在，它会打印 `No conversation found to continue` 并退出。使用 `claude --resume` 可以从列表中选择，或在运行中的会话内使用 `/resume`。请参阅[管理会话](/zh/sessions)了解命名、分支以及完整的选取器参考。
...
```
在第二个终端中使用不同名称运行相同命令，以启动一个隔离的并行会话。关于清理、`.worktreeinclude` 及非 Git VCS 支持，请参阅[工作树](/zh/worktrees)。要在一个屏幕中监控并行会话而非使用多个终端，请参阅[后台代理](/zh/agent-view)。
...
```
你也可以在会话中按 `Shift+Tab` 切换至计划模式。关于审批流程和在文本编辑器中编辑计划，请参阅[计划模式](/zh/permission-modes#analyze-before-you-edit-with-plan-mode)。
...
```
子代理在自身上下文窗口中读取文件并汇报摘要。如需定义自带工具和提示词的自定义代理，请参阅 [Subagents](/zh/sub-agents)。
...
```
参阅[非交互模式](/zh/headless)了解输出格式、权限标志和扇出模式。
